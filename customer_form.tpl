<!--  @dir admin/view//template/customer/customer_form.tpl -->
      ...
      <div class="panel-body" id="extension">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-customer" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <li><a href="#csv-add" data-toggle="tab">Import</a></li><!-- this is a new tab -->
            <?php } ?>
          </ul>
          ...
          <!-- here is added csv tab -->

            <div class="tab-pane" id="csv-add">
            <h2>Special price list for customer(<?php echo "$firstname $lastname"; ?>)</h2>
...
p{word-wrap: break-word;}
    ...
    <div id="impress"></div>
      <button type="button" id="button-upload" data-loading-text="Loading..." class="btn btn-primary"><i class="fa fa-upload"></i> Select (.csv) File</button>
             ...
             <?php if($time) echo "Latest import:<b> $time $editor[username] </b>";?>
  <table class="table table-bordered table-hover">
    <thead>
      <tr>
        <td class="text-right">Product Name</td>
        <td class="text-right">Product EAN Code</td>
        <td class="text-right">Special Price (EU)</td>
    ...
    ?>
      <tr id="<?= $row['id'];?>">
        ...
        <td class="text-right">
      <!--  <a href="#" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="Edit"><i class="fa fa-pencil"></i></a> -->
...
$('#extension').on('click', '.btn-danger, .btn-warning', function(e) {
	...
	row =$(this).attr('href');
		$.ajax({
      type: "POST",
			url: 'index.php?route=customer/csvPrice/delete&token=<?php echo $token; ?>',
		//	dataType: 'html',
			beforeSend: function() {
				$(node).button('loading');
			},
...
//$('#button-upload').on('click', function() {
  ...
  $('#extension').on('click','#button-upload' ,function() {
	$('#form-upload').remove();
	...
	if (typeof timer != 'undefined') {
    	clearInterval(timer);
            ...
            timer = setInterval(function() {
          ext = $('#form-upload input[name=\'file\']').val().split('.').pop().toLowerCase();
            ...
            image = new FormData($('#form-upload')[0]);
                ...
                $.ajax({
                  url: 'index.php?route=customer/csvPrice/addcsv&token=<?php echo $token; ?>&customer_id=<?= $customer_id?>',
                  type: 'post',
                  dataType: 'json',
                  cache: false,
                  //async: false,
     ...
     $(this).children().attr('contenteditable','false');
            ...
            */
            ...
            // alert( "button pushed" + value + row + collumn);
                ...
                $.ajax({
                type: "POST",
                url:"index.php?route=customer/csvPrice/update&token=<?php echo $token; ?>",
                data:  {'id': row, 
                        'collumn': collumn,
                        'value': value},
